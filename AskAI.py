...
class AskPlexMod(loader.Module):
    """
    Безкоштовний модуль для взаємодії з АІ.
    """
    ...
    async def message_q(
        ...
        """Відправляє повідомлення та очікує на повну відповідь, ігнорує проміжні статуси."""
        async with self.client.conversation(user_id, timeout=125) as conv:
            msg_to_bot = await conv.send_message(text)
            
            # Отримуємо першу відповідь
            response = await conv.get_response()

            # Перевірка: якщо це "статус" (хмаринка 💭 або текст Thinking...), то чекаємо наступне
            if response.text and ("💭" in response.text or "Thinking" in response.text):
                # Ігноруємо це повідомлення і беремо наступне (основне)
                response = await conv.get_response()
    ...
    async def аcmd(self, message: Message):
        ...
        # Отримуємо відповідь через оновлений message_q
