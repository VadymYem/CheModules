...
KEY_AUTO_RECOG = "auto_recognition_chats_author_vtt_v2"

@loader.tds
class AuthorVTTMod(loader.Module):
    """Розпізнавання голосу через Google, Gemini (AI) та Hugging Face з авто-режимом."""
    strings = {
        "name": "AuthorVTT",
        "pref": "<b>🎙️ AuthorVTT\n</b> ",
        "processing": "⏳ Обробка...",
        ...
        "cfg_hf_key": "Ключ API Hugging Face. Отримати: huggingface.co/settings/tokens",
        ...
        "hf_token_missing": "🚫 <b>Відсутній API-ключ Hugging Face.</b>\nНалаштуйте його в <code>.config</code>. Див. <code>.hfguide</code>.",
    ...
    # RU strings can be added here if needed, following the same structure

    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue("gemini_api_key", None, lambda: self.strings("cfg_gemini_key"), validator=loader.validators.Hidden()),
            loader.ConfigValue("hf_api_key", None, lambda: self.strings("cfg_hf_key"), validator=loader.validators.Hidden()),
    ...
    # HUGGING FACE RECOGNITION ENGINE
    async def _recognize_hf(self, audio_path: str, lang: str = None) -> str:
        if not self.config["hf_api_key"]: raise ValueError("hf_token_missing")
    ...
    async def _process_media(self, message: Message, engine: str, lang: str, status_msg: Message = None):
        ...
        try:
            ...
            # Prepare a universal audio format (mp3 for HF, wav for Google)
            ...
            if engine == "hf": return await self._recognize_hf(audio_for_api)
    ...
    async def _handle_recognition_command(self, message: Message, engine: str, lang: str = None):
        ...
        try:
            ...
            if engine == 'hf': source = "Hugging Face"

            text = self.strings("recognition_error")
            if error_key in ["no_reply", "too_big", "audio_extract_error", "gemini_token_missing", "gemini_lib_missing", "hf_token_missing"] or "too_short" in error_key:
    ...
    @loader.owner
    async def vhf(self, m: Message):
        """<відповідь> - Розпізнати мовлення (Hugging Face)"""
        await self._handle_recognition_command(m, "hf")
    ...
    async def _toggle_auto_recog(self, message: Message, engine: str, lang: str = None):
        ...
        if current == new:
            ...
            if engine == "hf":
                if not self.config["hf_api_key"]: return await utils.answer(message, self.strings("pref") + self.strings("hf_token_missing"))
            
            self._auto_recog_settings[chat_id] = new
            engine_map = {"google": self.strings("google_lang").format(lang), "gemini": self.strings("gemini"), "hf": self.strings("hf")}
    ...
    @loader.owner
    async def autohf(self, m: Message):
        """Перемкнути авто-розпізнавання (Hugging Face)"""
        await self._toggle_auto_recog(m, "hf")
    ...
    async def watcher(self, message: Message):
        ...
        try:
            ...
            if status_msg and not self.config["silent"]:
                ...
                if settings['engine'] == 'hf': source = "Hugging Face"
